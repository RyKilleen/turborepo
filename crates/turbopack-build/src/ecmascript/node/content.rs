...
use turbopack_core::{
    ...
    code_builder::{Code, CodeBuilder, CodeVc, OptionCodeVc},
...
use super::{
    content_entry::EcmascriptBuildNodeChunkContentEntriesVc,
    runtime::EcmascriptBuildNodeChunkRuntimeMode,
};
...
pub(super) struct EcmascriptBuildNodeChunkContent {
    ...
    pub(super) runtime_mode: EcmascriptBuildNodeChunkRuntimeMode,
}

impl EcmascriptBuildNodeChunkContent {
    async fn params(&self) -> Result<Option<Code>> {
        let exported_cjs_module_id = match self.runtime_mode {
            EcmascriptBuildNodeChunkRuntimeMode::Register => {
                return Ok(None);
            }
            EcmascriptBuildNodeChunkRuntimeMode::RegisterAndEvaluate => None,
            EcmascriptBuildNodeChunkRuntimeMode::RegisterAndExport(entry) => {
                Some(entry.as_chunk_item(self.chunking_context).id().await?)
            }
        };
...
impl EcmascriptBuildNodeChunkContentVc {
    ...
    pub(crate) async fn new(
        ...
        runtime_mode: Value<EcmascriptBuildNodeChunkRuntimeMode>,
    ) -> Result<Self> {
...
impl GenerateSourceMap for EcmascriptBuildNodeChunkContent {
    #[turbo_tasks::function]
    fn generate_source_map(self_vc: EcmascriptBuildNodeChunkContentVc) -> SourceMapVc {
